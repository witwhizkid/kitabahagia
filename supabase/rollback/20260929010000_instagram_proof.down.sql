-- Rollback for 20260929010000_instagram_proof.sql.
-- If the bucket contains proof files, archive/delete them before rollback.
begin;
...
do $$
begin
  if exists (select 1 from storage.objects where bucket_id = 'instagram-proofs') then
    raise exception 'instagram-proofs contains files; archive/delete them before rollback';
  end if;
  delete from storage.buckets where id = 'instagram-proofs';
end
$$;

alter table public.registrations drop column if exists instagram_proof_path;
...
$$;
revoke all on function public.create_registration(text, text, text, text, text, text, boolean, text, text, text, boolean) from public, anon, authenticated;
