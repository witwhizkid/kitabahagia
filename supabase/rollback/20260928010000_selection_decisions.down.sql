-- ROLLBACK for supabase/migrations/20260928010000_selection_decisions.sql
-- Not a migration (kept outside supabase/migrations so it never runs automatically).
-- Order:
--   1. Redeploy the previous admin-registrations, payment-status and admin-events
--      Edge Functions so nothing calls the new functions or columns.
--   2. Run this file in the SQL Editor. Waitlisted and rejected applicants go back to
--      'applied' (the status constraint below no longer allows them); accepted ones
--      stay 'confirmed'.
--   3. If the migration should be re-applied later, also run:
--      delete from supabase_migrations.schema_migrations where version = '20260928010000';
-- create_registration is restored verbatim from 20260927010000.

begin;

drop function if exists public.decide_selection(text[], text, uuid);
drop function if exists public.selection_overview(text);
...
update public.registrations set registration_status = 'applied'
 where registration_status in ('waitlisted', 'rejected');
...
alter table public.registrations
  drop column if exists selection_decided_at,
  drop column if exists selection_decided_by;

alter table public.events
  drop constraint if exists events_wa_message_length,
  drop column if exists wa_message_accepted,
  drop column if exists wa_message_waitlisted,
  drop column if exists wa_message_rejected;
