-- Rollback for 20260930010000_selection_extras.sql.
-- Stops if CVs were uploaded, so participant files are never deleted silently.
begin;

drop function if exists public.create_registration(text, text, text, text, text, text, boolean, text, text, text, boolean, text, text, text);

do $$
begin
  if exists (select 1 from storage.objects where bucket_id = 'selection-cvs') then
    raise exception 'selection-cvs contains files; archive/delete them before rollback';
  end if;
  delete from storage.buckets where id = 'selection-cvs';
end
$$;
...
alter table public.events drop column if exists selection_requirements;
...
$$;
revoke all on function public.create_registration(text, text, text, text, text, text, boolean, text, text, text, boolean, text) from public, anon, authenticated;
grant execute on function public.create_registration(text, text, text, text, text, text, boolean, text, text, text, boolean, text) to service_role;
