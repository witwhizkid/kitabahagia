-- Add optional participant details without invalidating existing registrations.
alter table public.registrations
  add column if not exists domicile text,
  add column if not exists institution text;

-- Replace the trusted registration RPC with an extended, backward-compatible
-- signature. The Edge Function supplies both nullable fields for new requests.
drop function if exists public.create_registration(text, text, text, text, text, text, boolean);
...
comment on function public.create_registration(text, text, text, text, text, text, boolean, text, text)
