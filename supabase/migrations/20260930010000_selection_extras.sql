-- Selection extras: per-event "Persyaratan jika lolos" list, and an optional CV /
-- portfolio (private PDF + https link) on free selection applications.
-- Existing callers keep working: the new RPC parameters default to NULL.
-- Rollback: supabase/rollback/20260930010000_selection_extras.down.sql

alter table public.events
  add column if not exists selection_requirements text[] not null default '{}';

-- Selection events already live showed this text as a static placeholder; keep it.
update public.events
   set selection_requirements = array[
 ...
 where registration_mode = 'selection' and selection_requirements = '{}';
...
values ('selection-cvs', 'selection-cvs', false, 5242880, array['application/pdf'])
on conflict (id) do update
set public = false,
    file_size_limit = excluded.file_size_limit,
    allowed_mime_types = excluded.allowed_mime_types;

-- No storage.objects policies: only Edge Functions (service_role) read or write CVs.
drop function if exists public.create_registration(text, text, text, text, text, text, boolean, text, text, text, boolean, text);
