-- Per-event switch for the CV/portfolio fields on free selection events, plus an
-- optional note shown under them. Same RPC signature; only the CV checks change.
-- Rollback: supabase/rollback/20261001010000_selection_cv_setting.down.sql

alter table public.events
  add column if not exists cv_requested boolean not null default false,
  add column if not exists cv_note text;

alter table public.events drop constraint if exists events_cv_note_length;
alter table public.events add constraint events_cv_note_length
  check (char_length(coalesce(cv_note, '')) <= 300);

-- Selection events that already exist showed the CV fields; keep them on.
update public.events set cv_requested = true where registration_mode = 'selection';
...
begin
  ...
  if v_cv_path is not null and (not v_selection or not v_event.cv_requested
  ...
  if v_portfolio_url is not null and (not v_selection or not v_event.cv_requested
